...
app.get('/json', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("course").find({}, {projection: {user: 1}}).toArray(function (err, result) {
            if (err) throw err;
            console.log(result);
            queryres = result;
            db.close().then(r => {
...
app.get('/currentuser', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("courses");
        dbo.collection("course").find({current: "true"}).toArray(function (err, result) {
            if (err) throw err;
            console.log(result);
            queryres = result;
            db.close().then(r => {
...
const accountSid = "ACaf667a2d55fe4118674b76162ca617d7";
const authToken = "efca9986f3b5ace5d19d661c65900443";
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("course").find({current: "true"}).toArray(function (err, result) {
            ...
            db.close().then(r => {
                ...
                client.verify.services.create({friendlyName: 'NPTEL Course Registration'})
                    .then(service => {
...
app.post('/login', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("courses");
        dbo.collection("course").find({user: user}, {projection: {password: 1}}).toArray(async function (err, result) {
            if (err) {
                res.send(false);
                throw err;
            }
...
app.get('/logout', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("course").find(myquery, {projection: {user: 1}}).toArray(async function (err, result) {
            if (err) {
                res.send(false);
                throw err;
            }
...
app.post('/signup', async function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("courses");
        dbo.collection("course").insertOne(redt, function (err, res) {
            if (err) throw err;
